...
<!-- LOCAL ADDITIONS -->
<xsl:template match="html:span[@class='tru']">
  <phrase role="logical-vowel">
    <xsl:apply-templates select="node()|comment()|text()|processing-instruction()"/>
  </phrase>
</xsl:template>
<xsl:template match="html:span[@class='c3']">
  <phrase role="IPA">
    <xsl:apply-templates select="node()|comment()|text()|processing-instruction()"/>
  </phrase>
</xsl:template>
<xsl:template match="html:td[@class='c3']">
  <entry>
    <phrase role="IPA">
      <xsl:apply-templates select="node()|comment()|text()|processing-instruction()"/>
    </phrase>
...
<xsl:template match="html:span[@class='sumti']">
  <foreignphrase xml:lang="jbo" role="sumti">
    <xsl:apply-templates select="node()|comment()|text()|processing-instruction()"/>
  </foreignphrase>
</xsl:template>
<xsl:template match="html:span[@class='selbri']">
  <foreignphrase xml:lang="jbo" role="selbri">
    <xsl:apply-templates select="node()|comment()|text()|processing-instruction()"/>
  </foreignphrase>
</xsl:template>
...
<xsl:template match="html:a[contains(@href,'#')]" priority="1.0">
 <xref>
...
<xsl:template match="html:a[contains(@href,'mailto:')]" priority="1.0">
 <link>
  <xsl:attribute name="xlink:href">
   <xsl:value-of select="normalize-space(@href)"/>
  </xsl:attribute>
...
<!-- These two templates are exactly the same, except one uses a's
     id and the other a's name; please change both if you change
     either.

     BEGIN two mostly-identical templates 
     -->
<xsl:template match="html:a[@id != '']" priority="0.6">
 <anchor>
  <xsl:attribute name="xml:id">
   <xsl:call-template name="make_id">
...
<!-- END two mostly-identical templates -->

<!-- Match picture links -->
<xsl:template match="html:a[contains(@href,'.gif') or contains(@href,'.svg') or contains(@href,'.png') or contains(@href,'.jpg')]" priority="2.0">
  <mediaobject>
    <alt>
...
<xsl:template match="html:a[@href != '']" priority="0.8">
 <xref>
  <xsl:attribute name="linkend">
   <xsl:value-of select="$prefix"/>
   <xsl:text>_</xsl:text>
...
<xsl:template match="html:div[contains(@style,'margin-left: 2em')]">
 <blockquote><para>
  <xsl:apply-templates select="node()|comment()|text()|processing-instruction()"/></para>
 </blockquote>
</xsl:template>

<xsl:template match="html:a[@href != '' 
                      and not(boolean(ancestor::html:h1|ancestor::html:h2|ancestor::html:h3|ancestor::html:h4|ancestor::html:h5|ancestor::html:h6|ancestor::html:dd|ancestor::html:pre|ancestor::html:dt|ancestor::html:p|ancestor::html:li))]" 
              priority="1">
 <para>
 <xref>
  <xsl:attribute name="linkend">
...
<xsl:template match="html:a[contains(@href,'#') 
                    and not(boolean(ancestor::html:h1|ancestor::html:h2|ancestor::html:h3|ancestor::html:h4|ancestor::html:h5|ancestor::html:h6|ancestor::html:dd|ancestor::html:p|ancestor::html:li))]" 
              priority="1.1">
 <para>
 <xref>
  <xsl:attribute name="linkend">
...
<!-- Normal tables; no thead or tbody -->
<xsl:template match="html:table[not(boolean(child::html:thead))]">
...
<xsl:template match="html:table[boolean(child::html:thead)]">
<!-- <xsl:variable name="column_count"
               select="saxon:max(.//html:tr,saxon:expression('count(html:td)'))"/> -->
 <xsl:variable name="column_count">
  <xsl:call-template name="count_columns">
 ...
 <informaltable>
  <tgroup>
   ...
   <thead>
    <xsl:apply-templates select="html:thead/html:tr[1]"/>
   </thead>
   <tbody>
